...
class BoundedPriorityQueue:
    ...
    def add(self, item: any, priority: int) -> None:
        ...
        if priority >= len(self._queues):
            raise ValueError("add(item, priority):", priority, "is an invalid priority level")
    ...
    def remove(self) -> any:
        ...
        """
        if len(self) == 0:
            raise IndexError("builtins.IndexError: remove from an empty BoundedPriorityQueue")
        
        for index in range(len(self._queues)):
            if len(self._queues[index]) > 0:
                self._num_items -= 1
