...
class ArrayList:
    ...
    def extend(self, iterable) -> None:
        ...
        if len(self) == 0:
            length = len(iterable)
            iterable_list = ArrayList([])
            iterable_list._elems = _new_array(length)
            for elem in iterable:
                iterable_list.append(elem)
            for elements in iterable_list:
        ...
        self_list._elems = _new_array(length_self)
        for item in self:
        ...
        self._elems = _new_array(length_self + length)
        self._num_items = 0
        
        for items in self_list:
            self.append(items)
    ...
    def pop(self, i: int = -1) -> any:
        ...
        if 0 <= i < len(self):
            item = self._elems[i]
            self._elems[i: len(self) - 1] = self._elems[i + 1: len(self)]
            self._num_items -= 1
            return item
        else:
