...
class BinaryMaxHeap:
    ...
    def remove(self) -> any:
        ...
        """
        if len(self) == 0:
            raise IndexError("remove cannot work on an empty heap")
       
        x = self._elems[0]
        self._elems[0] = self._elems[len(self)-1]
        self._elems = self._elems[0:len(self)-1]
    ...
    def _trickle_down(self, i: int) -> None:
        ...
        while i >= 0:
            ...
            r = right(i)
            if r < size and self._elems[r] < self._elems[i]:
                l = left(i)
                ...
                l = left(i)
                if l < size and self._elems[l] < self._elems[i]:
                    j = l
